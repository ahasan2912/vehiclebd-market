"use client"
import { Orders } from '@/app/(dashboard)/user-dashboard/orderlist/components/OrderList';
...
const OrderList = ({ orders }: { orders: Orders[] }) => {
    ...
    return (
        <div className="p-8 rounded-lg shadow-2xl">
            <h2 className="text-4xl font-extrabold text-gray-900 mb-6 border-b-2 border-blue-500 pb-4">All Orders</h2>
                ...
                <table className="min-w-full overflow-x-auto">
                    <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th className="py-4 px-6 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Customer Address</th>
                        ...
                        {orders.length > 0 ? (
                            orders.map((order, index) => (
                                <tr key={order._id} className={`transition-colors duration-150 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-indigo-50`}>
                                    ...
                                    <td className="py-4 px-6 whitespace-nowrap text-sm text-left  text-gray-900 font-medium">{order.buyerName}</td>
                                    ...
                                    <td className="py-4 px-6 whitespace-nowrap text-gray-900 font-medium text-sm text-left">{order.buyerAddress}</td>
                                    <td className="py-4 text-sm px-6 whitespace-nowrap text-gray-900 font-semibold">{new Date(order.orderDate).toLocaleDateString()}</td>
